...
2. Download the necessary libraries and python3 to execute correctly the system:
```bash
sudo apt update
...
```
obtaining a cluster with rabbit (listening on port 5672 by default), rabbit2 (listening on port 5673) and rabbit3 (listening on port 5674) machines. The first one, is already created by the installation at point 1.
...
5. Using the terminal, move inside the newly cloned repository and then you can start the system with the start.sh file, whose syntax is this:
```bash
